...
module.exports = async (req, res) => {
  try {
    ...
    if (req.method === "GET") {
      let query = {};
      if (categoryId) {
        query.categoryId = categoryId; // Filtrujemy po categoryId
      }
      const parts = await partsCollection.find(query).toArray();
      return res
        .status(200)
        .json(parts.map((part) => ({ ...part, id: part._id.toHexString() })));
    }

    if (req.method === "POST") {
      const newPart = req.body;
      const result = await partsCollection.insertOne(newPart);
      const createdPart = { ...newPart, id: result.insertedId.toHexString() };
